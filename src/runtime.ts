/**
 * pi-agent-router runtime.
 *
 * Heavy runtime implementation loaded lazily by the lightweight extension registrar.
 */
...
} from "./task/task-registry-persistence";
...
const MODEL_DISCOVERY_READY_EVENT = "pi-model-discovery:ready";
...
export default function agentRouterExtension(pi: ExtensionAPI) {
  ...
  let loadedTaskRegistryNeedsSave = false;
  for (const entry of loadPersistedTaskRegistryEntries()) {
    if (entry.status === "running" || entry.status === "queued") {
      entry.status = "aborted";
      entry.updatedAt = Date.now();
      entry.lastError = entry.lastError
        ? `${entry.lastError}\nExtension restarted before delegated task completed.`
        : "Extension restarted before delegated task completed.";
      entry.lastExitCode = undefined;
      entry.lastTimedOut = false;
      loadedTaskRegistryNeedsSave = true;
    }
    subagentTaskRegistry.set(entry.taskId, entry);
  }

  const saveTaskRegistry = (): void => {
    writePersistedTaskRegistry(subagentTaskRegistry.values());
  };
  if (loadedTaskRegistryNeedsSave) {
    saveTaskRegistry();
  }
  ...
  const shutdownTrackedSubagentSessions = (ctx: ExtensionContext): boolean => {
    ...
    for (const parentSessionId of parentSessionIds) {
      shutdownChangedState =
        ...
        shutdownChangedState;
      releaseSubagentAuthForParentSession(parentSessionId);
    }
  ...
  const pruneFinishedSessions = (): number => {
    ...
    for (const [taskId, tracked] of subagentTaskRegistry.entries()) {
      const retainedChildSessionIds = tracked.childSessionIds.filter((childId) =>
        subagentSessions.has(childId),
      );
      if (retainedChildSessionIds.length !== tracked.childSessionIds.length) {
        tracked.childSessionIds = retainedChildSessionIds;
  ...
  const isSuccessfulTerminalStopReason = (value: string | undefined): boolean => {
    const normalized = normalizeInputText(value).toLowerCase();
    return normalized === "stop" || normalized === "done";
  };

  const getLatestAssistantErrorMessage = (
  ...
  const hasOnlyNonFatalDelegatedStderr = (stderr: string): boolean => {
    ...
    return lines.every((line) => /^Warning: No models match pattern ".+"$/i.test(line));
  };

  const hasTerminalAssistantCompletionOutput = (run: SubagentRunResult): boolean => {
    if (!isSuccessfulTerminalStopReason(getLatestAssistantStopReason(run))) {
      return false;
    }

    if (getLatestAssistantErrorMessage(run)) {
      return false;
    }
  ...
  const shouldTreatNonZeroExitAsSuccessfulCompletion = (run: SubagentRunResult): boolean =>
    run.code !== 0 &&
    !run.timedOut &&
    hasTerminalAssistantCompletionOutput(run) &&
    hasOnlyNonFatalDelegatedStderr(run.stderr);
  ...
  const finalizeSession = (
    ...
    const nonFatalCompletedExit = shouldTreatNonZeroExitAsSuccessfulCompletion(run);
    ...
    if (session.status !== "killed") {
      if (session.timedOut || run.timedOut) {
        ...
        session.status = "failed";
      } else if (run.code === 0 || nonFatalCompletedExit) {
    ...
    if (nonFatalCompletedExit) {
      session.outputNotice = mergeOutputNotices(
        session.outputNotice,
        `Delegated process exited ${run.code} after a terminal assistant completion; treating as successful because stderr contained only nonfatal model-scope warnings.`,
      );
    }
    ...
    const retainedCompletedError = buildRetainedHistoryText(
      selectRetainedCompletionErrorText({
        terminalFailure,
        failureSummarySource,
        sessionStderr: session.stderr,
  ...
  const startBackgroundSubagent = async (
    ...
    if (delegatedModelResolution.requested && !delegatedModel) {
      return failStartup(
        ...
        "Use --list-models to verify availability or retry after pi-model-discovery finishes registering providers.",
      );
    }
    ...
    let preparedSubagentAuth: Awaited<ReturnType<typeof prepareSubagentAuthForLaunch>> | undefined;
    ...
    try {
      ...
      preparedSubagentAuth = await prepareSubagentAuthForLaunch({
      ...
      if (preparedSubagentAuth.failureMessage) {
        throw new Error(preparedSubagentAuth.failureMessage);
      }

      const directEnvAuthAvailable =
        preparedSubagentAuth.mode === "direct-env" ||
      ...
      for (const extensionDir of preparedSubagentAuth.extensionDirs) {
        if (!delegatedExtensionDirs.includes(extensionDir)) {
          delegatedExtensionDirs.push(extensionDir);
        }
      }
      ...
      if (typeof agent.temperature === "number") {
        ...
        runtimeArtifacts.push({
          ...
          source: buildDelegatedTemperatureExtensionSource(agent.temperature),
          requiredForLaunch: true,
        });
      }
    ...
    const LOCK_ERROR_PATTERN = new RegExp(
      [
        "Lock file is already being held",
        "ELOCKED",
        "(?:EPERM|EACCES|EBUSY): operation not permitted, rename .*pi-model-discovery.*cache\\.json",
        "(?:EPERM|EACCES|EBUSY).*pi-model-discovery.*cache\\.json",
      ].join("|"),
      "i",
    );
    ...
    const hasExtensiveDelegatedProgress = (run: SubagentRunResult): boolean =>
      getNormalizedRunOutputText(run).length > 0 || getRunToolInvocationCount(run) > 0;
    ...
    const getRetryableDelegatedAuthFailure = (run: SubagentRunResult) => {
      if (run.timedOut) {
        return undefined;
      }

      const failureText = getRunFailureText(run);
      if (!failureText.trim()) {
        return undefined;
      }

      const latestAssistantError = getLatestAssistantErrorMessage(run);
      const latestStopReason = normalizeInputText(getLatestAssistantStopReason(run)).toLowerCase();
      const silentSuccessfulFailure = getSilentSuccessfulRunMessage(run);
      const hasAssistantErrorStop = latestStopReason === "error";
      const hasSuccessfulCompletion = hasTerminalAssistantCompletionOutput(run);

      if (
        run.code === 0 &&
        !latestAssistantError &&
        !hasAssistantErrorStop &&
        !silentSuccessfulFailure
      ) {
        return undefined;
      }
      ...
      return classifyDelegatedRetryableFailure(failureText, {
        providerId: subagentProviderId,
        modelId: delegatedModelContext?.modelId ?? delegatedModelRef,
      });
    };
    ...
    let nextRetrySessionPath: string | undefined;

    const runSubagentProcess = async (
      stdinText: string,
      continuationSessionPath?: string,
    ): Promise<SubagentRunResult> => {
      const activePreparedSubagentAuth = preparedSubagentAuth;
      if (!activePreparedSubagentAuth) {
        return {
          code: 1,
          stdout: "",
          stderr: "[pi-agent-router] Delegated auth preparation did not complete before subagent launch.",
          timedOut: false,
          sessionPath: options.sessionPath,
        };
      }
        ...
        completeAttempt = (code: number): void => {
          ...
          void (async () => {
            try {
              ...
              void reportSubagentAuthAttemptResult(activePreparedSubagentAuth, {
                providerId: subagentProviderId,
                modelId: delegatedModelContext?.modelId,
                modelRef: delegatedModelRef,
                api: delegatedModelContext?.api,
                parentSessionId: session.parentSessionId,
                subagentSessionId: session.id,
                exitCode: code,
                timedOut: Boolean(session.timedOut),
                stderr: stderrSink.summarize().tailText,
              });
              releaseSubagentAuthForLaunch(activePreparedSubagentAuth, {
                parentSessionId: session.parentSessionId,
                subagentSessionId: session.id,
                providerId: subagentProviderId,
              });
        ...
        const startSubagentProcess = async (): Promise<void> => {
          ...
          try {
            ...
            const subagentEnv = buildSubagentSpawnEnv({
              ...
              delegatedAuthRuntimeDir: AGENT_DIR,
              permissionPolicyAgentDir: AGENT_DIR,
              modelDiscoveryCacheOnly: Boolean(delegatedModelRef),
              inheritedEnvKeys: activePreparedSubagentAuth.inheritedEnvKeys,
              delegatedAuthEnv: activePreparedSubagentAuth.env,
            });
    ...
    const runSubagentAttempt = async (): Promise<SubagentRunResult> => {
      ...
      const retrySessionPath = nextRetrySessionPath;
      nextRetrySessionPath = undefined;
      ...
      while (
        ...
        const continuationRun = await runSubagentProcessWithRuntimeArtifactRetry("", continuationSessionPath);
        run = mergeContinuationRunResult(run, continuationRun);
        await continueAfterOverflowCompaction();
      }
    ...
    void subagentProcessLifecycle.runWithRetry({
      ...
      getErrorMessage,
      getRetryableCredentialFailure: getRetryableDelegatedAuthFailure,
      getRunFailureText,
      preferRicherRunResult,
      isLockRetryableFailure,
      sleep,
      getLastAcquiredCredentialId: () => undefined,
      transientRetrySettings: options.retrySettings,
      prepareRetryAttempt: async ({ run, failure, attempt, maxAttempts }) => {
        const retrySessionPath = run.sessionPath || session.sessionPath;
        if (!retrySessionPath) {
          return;
        }
        ...
        if (sessionIntegrity.hasPendingToolCalls || sessionIntegrity.error) {
          ...
          void piAgentRouterDebugLogger.warn("subagent.retry_session_clean_start", {
            message:
              `[pi-agent-router] Starting clean auto-retry for delegated task ${session.id.slice(0, 8)} ` +
              `because retained session cannot be safely resumed: ${reason}.`,
        ...
        nextRetrySessionPath = retrySessionPath;
        void piAgentRouterDebugLogger.info("subagent.retry_session_resume", {
          message:
            `[pi-agent-router] Resuming delegated session ${session.id.slice(0, 8)} for auto-retry ` +
            `${attempt}/${maxAttempts}.`,
          attempt,
          failureKind: failure.kind,
          maxAttempts,
      ...
      onRetry: (event) => {
        const retryLogEvent = event.kind === "lock"
          ? "subagent.lock_contention_retry"
          : "subagent.delegated_retry_scheduled";
        void piAgentRouterDebugLogger.warn(retryLogEvent, {
  ...
  const syncManualThinkingOverrideForAgent = async (
    ctx: ExtensionContext,
    agentName: string | null,
  ): Promise<void> => {
    if (!agentName) {
    ...
    const configuredAgent = (await loadAgentsAsync({ cwd: ctx.cwd, scope: "both" })).find(
  ...
  const setActiveAgent = (
    ...
    const loadedAgents = activeAgent
      ? options.loadedAgents ?? loadAgents({ cwd: ctx.cwd, scope: "both" })
      : [];

    if (options.notify) {
      if (activeAgent) {
        ctx.ui.notify(
          `${getAgentEmoji(activeAgent, loadedAgents)} Active agent: ${activeAgent}`,
          "info",
        );
      } else {
  ...
  const cycleAgent = (ctx: ExtensionContext, direction: 1 | -1): void => {
    ...
    if (cyclable.length === 0) {
      ...
      ctx.ui.notify(
        `No primary agents found for Tab cycling. Add mode: primary or mode: all to agent frontmatter.\nAvailable: ${available}`,
        "warning",
      );
      return;
    }
    ...
    setActiveAgent(ctx, cyclable[nextIndex], {
      persist: true,
      notify: false,
      loadedAgents: agents,
    });
  ...
  const restoreActiveAgentFromSession = async (ctx: ExtensionContext): Promise<void> => {
    ...
    const agents = await loadAgentsAsync({ cwd: ctx.cwd, scope: "both" });
    const targetAgentName = persisted ?? DEFAULT_AGENT;
    const persistedAgent = agents.find((agent) => agent.name === targetAgentName);
    if (!persistedAgent) {
      activeAgent = null;
      clearAgentStatus(ctx);
      ctx.ui.notify(
        `Saved agent '${targetAgentName}' was not found. Agent mode is disabled.`,
        "warning",
      );
      return;
    }

    setActiveAgent(ctx, persistedAgent.name, {
      persist: false,
      notify: false,
      applyRuntime: false,
      loadedAgents: agents,
    ...
    if (ctx.hasUI) {
      await applyAgentRuntimeProfile(ctx, persistedAgent, {
        respectManualThinkingOverride: true,
      });
    }
  };
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (event.reason === "new" || event.reason === "resume" || event.reason === "fork") {
      ...
      await restoreActiveAgentFromSession(ctx);
  ...
  pi.on("input", async (_event, ctx) => {
    await syncManualThinkingOverrideForAgent(ctx, activeAgent);
  });
  ...
  pi.registerCommand("agent", {
    ...
    handler: async (args, ctx) => {
      ...
      if (ctx.hasUI) {
        ...
        setActiveAgent(ctx, selectedAgentName, {
          persist: true,
          notify: true,
          loadedAgents: agents,
        });
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    const agent = (await loadAgentsAsync({ cwd: ctx.cwd, scope: "both" })).find((item) => item.name === activeAgent);
    if (!agent) {
      activeRuntimeTemperature = undefined;
      return {};
    }
  ...
  const taskToolDefinition = defineTool({
    ...
    description: buildTaskToolDescription(
      "Agent catalog discovery is resolved lazily when the task tool runs.",
    ),
            ...
            worker: async (task, index, workerSignal) => {
              ...
              try {
                ...
                const finalStatus: SubagentExecutionStatus =
                  workerSignal.aborted || latestSessionStatus === "killed"
                    ...
                    : run.timedOut || Boolean(session.timedOut)
                      ? "timed_out"
                      : run.code === 0 || nonFatalCompletedExit
                        ? "finished"
                        : "failed";
                ...
                if (nonFatalCompletedExit) {
                  taskResult.contractWarnings = [
                    ...(taskResult.contractWarnings || []),
                    `Task ${taskResult.index}: delegated process exited ${run.code} after a terminal assistant completion; treated as successful because stderr contained only nonfatal model-scope warnings.`,
                  ];
                }
